...
class Blockchain():
    ...
    def verify(self,block): #input:block, output:True(bool),(str)
        ...
        if block.prev_hash != self.chain[-1].get_blockhash():
            print("[verify]hash fake")
            return "hash fake"

        """
        #timestamp fake
        if self.chain[-1].timestamp > block.timestamp:
            print("[verify]timestamp fake")
            return "timestamp fake"
        """

        for _ in self.chain:
            result = _.cmp(block)   #ex. ["id","timestamp"]
            
            #same id
            """
            if Block().attritube[0] in result:
                print("[verify]same id")
                return False
            """


            #print("[verify]pass same exam")

            list = [False,True,True,True,True,True] # id\timestamp\nonce\prev_hash\information\source
            count = 0
            for attritube in block.attritube:
                #print("[verify]attritube in result = ",attritube in result)
                if (attritube in result) == list[count]:
                    count += 1
                if count >= 6:
                    print("[verify]all same")
                    return "all same"
